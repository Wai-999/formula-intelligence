...
# Fifth Pass — Model Map: broken landing view + detail panel crash on every click

Reported live by the app's user: opening Model Map showed nothing usable ("landing style" didn't match Stats mode's Map), and clicking any node did nothing at all. Both were regressions/gaps invisible to every prior pass's verification, because this environment had no browser until this pass — a working headless Chromium (Playwright, manually patched around a missing shared lib since there's no root to `apt install` it) was set up specifically to chase these two reports against the real rendered app instead of static analysis alone.

## F.1 — Model Map's landing view rendered off-screen at ~10x zoom

**Root cause confirmed live, not just by reading the code.** `MLBody.jsx` keep-alive-mounts every ML tab and hides inactive ones with `display:none`; `mlActiveTab` defaults to `'pipeline'` and is never persisted, so Model Map is `display:none` at the exact moment `MLRelationshipMap.jsx` mounts and calls `contentGroup.node().getBBox()` to compute its fit-to-view bounds. `getBBox()` returns a degenerate near-zero box for anything inside a `display:none` ancestor. That degenerate box drove `computeFit()` into an enormous scale centered near the origin — every node was technically rendered, just zoomed/panned far outside the visible viewport. This is exactly why Model Map's landing looked nothing like Stats mode's Map: Stats' Map is `activeTab`'s own default (always visible at first mount, `getBBox()` never involved — its fit uses `computeHierarchicalLayout()`'s static, hand-computed bounds instead), so it never hits this path at all.

**Fixed:** turned the one-time `const bounds = {...}` (built from a single `getBBox()` call at mount) into a `measureBounds()` function and a reassignable `let bounds`, then re-measure inside the existing `onResize` `needsRefit` branch — the point at which the pane's wrapper `width`/`height` were already being re-read for the same "was hidden, now visible" reason. Confirmed live: first node's circle bounding box went from `{x: 4957, y: 318, width: 255, height: 255}` (off-screen, ~15x too large) before the fix to `{x: 559.9, y: 115.8, width: 16.9, height: 16.9}` (on-screen, correctly scaled) after — verified by loading the actual dev server in headless Chromium, switching to ML mode, opening Model Map, and reading the rendered SVG's real geometry.

**Files touched:** `src/components/mlgraph/MLRelationshipMap.jsx` (`measureBounds()` + one re-measurement call, ~15 net new lines including comments).

## F.2 — Clicking any Model Map node crashed the whole app (silently, since there's no error boundary)

**Root cause confirmed live via the browser's own uncaught-exception event, not inferred.** `MLModelDetailPanel.jsx` rendered `{family?.name}` directly as a JSX child, where `family` comes from `ML_FAMILIES.find((f) => f.id === node.ch)` in `models.js` — and every `ML_FAMILIES` entry's `name` is a `blSame(en, my)` object (`{en:{beginner,researcher}, my:{beginner,researcher}}`), not a string, exactly the bl()-object-rendered-raw bug class this app's own docs warn about. React throws `Objects are not valid as a React child (found: object with keys {en, my})` the instant this renders — which is every time, for every node, since every model has a valid family. With no error boundary anywhere in this app (documented in `docs/ML_MODE_README.md`), that throw doesn't just fail the detail panel — it blanks the entire UI. This is why the user saw literally nothing happen on click: the click handler was working correctly (`selectModel` fired, the panel began to render), it just crashed immediately afterward, invisibly.

Every other model/family/chapter field rendered elsewhere in the app (`node.name`, `ch.name`, etc.) was re-checked against its source data and confirmed to be a plain string, not bl()-wrapped — this was the only such raw-render bug in the codebase (confirmed by an AST-restricted sweep of `bl()`/`blSame()` call sites plus a targeted grep for every `.name`/`?.name`/`.title`/`.label` JSX expression across `src/`).

**Fixed:** added `const familyName = useT(family?.name);` alongside this component's other `useT()` calls (all called unconditionally, before the `if (!node) return` early exit, matching the existing pattern and keeping hook-call order stable), and moved the `family` lookup itself up to sit next to `node`/`ladder` for the same reason. Changed the JSX to render `{familyName}` instead of `{family?.name}`.

**Files touched:** `src/components/mlgraph/MLModelDetailPanel.jsx` (~13 net new/changed lines, one `useT()` call added, `family` computation relocated, one JSX render target changed).

**Verification (both F.1 and F.2):** `npm run build`/`npm run lint` clean. All five static/data-model checks from the Third Pass re-run clean. Then, for the first time in this project's fix history, verified end-to-end against the actual running app in headless Chromium: switched to ML mode, dismissed the first-visit misconception banner, opened Model Map, confirmed all 32 nodes render at correct on-screen positions (F.1), clicked the first node, confirmed `.ml-detail-panel.open` appears with real content (family badge "Linear & Regularized", formula, how-it-works, advantages, weaknesses, usage areas, compass meters, all four depth-ladder tabs) and zero console errors and zero uncaught page exceptions (F.2) — screenshotted both states as evidence.
